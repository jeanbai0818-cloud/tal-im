...
群聊默认策略为 **`pairing`**（与私信一致）：机器人被加入新群后不会立即响应，需管理员通过 `openclaw channels approve` 审批该群，之后群内成员才能与机器人交互。

支持的 `groupPolicy` 值：

| 值 | 说明 |
|----|------|
| `pairing`（默认） | 新群须管理员审批后才可交互 |
| `allowlist` | 仅 `groupAllowFrom` 白名单内的群 |
| `open` | 机器人所在的所有群均可触发 |
| `disabled` | 拒绝所有群消息 |
